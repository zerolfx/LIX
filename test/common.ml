...
| Some (Lix.Ast.Int y) -> assert_equal x y
| _ -> assert false


let assert_eval_bool x s = fun _ -> match eval s |> list_last_element with
| Some (Lix.Ast.Bool y) -> assert_equal x y
| _ -> assert false


let assert_eval_all xs s = fun _ ->
  let ys = eval s in
  let rec check (xs : Lix.Ast.primitive list) (ys : Lix.Ast.primitive option list) : unit = match xs, ys with
  | _, None::yy -> check xs yy
  | [], [] -> ()
  | _, [] -> ()
  | [], _ -> ()
  | x::xx, Some y::yy -> assert_equal ~cmp:Lix.Ast.equal_primitive x y; check xx yy in
  check xs ys
